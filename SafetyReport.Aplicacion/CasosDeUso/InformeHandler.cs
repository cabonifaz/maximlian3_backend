...
using SafetyReport.Domain.Informes;
    ...
    public class InformeHandler
    {
                ...
                foreach (var imagen in local.Imagenes)
                    if (imagen.IdInformeLocalImagen is null or 0)
                    {
                        if (string.IsNullOrWhiteSpace(imagen.Nombre))
                            return InformeImagenReglas.MensajeNombreRequerido;

                        if (!InformeImagenReglas.EsExtensionPermitida(imagen.Nombre))
                            return InformeImagenReglas.MensajeExtensionNoPermitida(imagen.Nombre);
                    }
            return null;
        }
        ...
        public async Task<Respuesta> EnviarNotificacionInformeAsync(UsuarioGeneral usuarioLogueado, InformeIdRequest request)
        {
            try
            {
                ...
                {
                    var extension = InformeDocumentoReglas.MapearExtensionFormato(formatoLabel);
                    if (extension == null)
                    {
        ...
        private async Task<EmailAdjunto?> ObtenerAdjuntoDocumentoAsync(UsuarioGeneral usuarioLogueado, int idInforme, int idPedido, string formato)
        {
            ...
            return new EmailAdjunto
            {
                ...
                ContentType = InformeDocumentoReglas.MapearContentType(formato),
