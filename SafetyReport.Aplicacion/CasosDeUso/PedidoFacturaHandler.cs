...
using SafetyReport.Domain.Archivos;
using SafetyReport.Domain.Facturacion;
...
namespace SafetyReport.Application.CasosDeUso
{
    public class PedidoFacturaHandler
    {
        ...
        // se resuelve aparte (ClienteRepositorioSql) en vez de tomarlo de la primera fila — así el nombre del archivo
        ...
        public async Task<Respuesta> ExportarPedidosParaPrefacturaAsync(UsuarioGeneral usuarioLogueado, FiltroPedidoPrefactura request)
        {
            try
            {
                ...
                var archivo = _prefacturaExcelExporter.GenerarExcelPrefactura(resultado.Headers, resultado.Items);
                var etiquetaPeriodo = ObtenerEtiquetaPeriodo(request);
                var nombreArchivo = $"{NombreArchivoReglas.Sanitizar(nombreCliente, "CLIENTE")} List of Reports {etiquetaPeriodo}.xlsx";
        ...
        public async Task<Respuesta> EmitirFacturaAsync(UsuarioGeneral usuarioLogueado, int idDocumentoElectronico)
        {
            try
            {
                ...
                var idEstadoFacturacion = FacturacionReglas.MapearEstadoFacturacionSunat(resultado.Datos.EstadoCodigo);
        ...
        public async Task<Respuesta> AnularFacturasAsync(UsuarioGeneral usuarioLogueado, AnularFacturasRequest request)
        {
            try
            {
                ...
                var itemsBoleta = tipos.Where(t => FacturacionReglas.EsBoleta(t.TipoDocumentoCodigo, t.Referencia?.TipoDocumentoRelacionadoCodigo)).Select(t => t.Item).ToList();
                var itemsOtros = tipos.Where(t => !FacturacionReglas.EsBoleta(t.TipoDocumentoCodigo, t.Referencia?.TipoDocumentoRelacionadoCodigo)).Select(t => t.Item).ToList();
        ...
        public async Task<Respuesta> PrevisualizarBajaAsync(UsuarioGeneral usuarioLogueado, List<int> idsDocumentoElectronico)
        {
            try
            {
                ...
                var idsBoleta = tipos.Where(t => FacturacionReglas.EsBoleta(t.TipoDocumentoCodigo, t.Referencia?.TipoDocumentoRelacionadoCodigo)).Select(t => t.Id).ToList();
                var idsOtros = tipos.Where(t => !FacturacionReglas.EsBoleta(t.TipoDocumentoCodigo, t.Referencia?.TipoDocumentoRelacionadoCodigo)).Select(t => t.Id).ToList();
